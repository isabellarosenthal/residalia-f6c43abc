...
export function AreasGrid({ edificioId, onEdit }: { edificioId: string; onEdit: (a: AreaComun) => void }) {
  ...
  if (isLoading) return <div className="text-center text-[#8b8bb5] py-10">Cargando…</div>;
  if (areas.length === 0) return <div className="text-center text-[#8b8bb5] py-10">Sin áreas comunes. Crea la primera.</div>;
        ...
        <Card key={a.id} className="p-5">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-11 h-11 rounded-xl bg-[#ddd6fe] text-[#818cf8] flex items-center justify-center shrink-0"><Sparkles className="w-5 h-5" /></div>
              <div className="min-w-0">
                <div className="font-display font-bold text-lg text-[#1e1b4b] truncate">{a.nombre}</div>
          ...
          <div className="mt-4 space-y-1.5 text-sm text-[#5a3a25]">
            {a.capacidad ? <div className="flex items-center gap-2"><Users className="w-4 h-4 text-[#8b8bb5]" />Capacidad: {a.capacidad}</div> : null}
            {(a.horario_inicio || a.horario_fin) && <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-[#8b8bb5]" />{a.horario_inicio?.slice(0,5) ?? "—"} – {a.horario_fin?.slice(0,5) ?? "—"}</div>}
          </div>
        </Card>
      ))}
    </div>
