  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle className="font-display text-xl text-[#4F46E5]">{reserva ? "Editar reserva" : "Nueva reserva"}</DialogTitle></DialogHeader>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={save.isPending || !!conflicto} className="bg-[#4F46E5] hover:bg-[#4338CA]">{save.isPending ? "Guardando…" : "Guardar"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
