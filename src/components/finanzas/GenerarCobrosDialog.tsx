  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) { setStage("form"); setExcluded(new Set()); } onOpenChange(v); }}>
      <DialogContent className="sm:max-w-[640px] max-h-[85vh] overflow-y-auto">
        <DialogHeader><DialogTitle className="font-display text-xl text-[#173B7A]">Generar cobros mensuales</DialogTitle></DialogHeader>
            ...
            {!isLoading && (
              <>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="font-display font-bold text-[#173B7A]">{fmtL(total)}</div>
                </div>
                  ...
                  {rows.map((r) => (
                    <label key={r.unidad_id} className={`flex items-center gap-3 p-3 ${r.duplicado ? "opacity-60 bg-[#fafafa]" : "hover:bg-[#fbf6f3] cursor-pointer"}`}>
                      ...
                      <div className="flex-1">
                        <div className="text-sm font-medium text-[#173B7A]">Unidad #{r.unidad_numero}</div>
                      ...
                      <div className="text-sm font-semibold text-[#173B7A] tabular-nums">{fmtL(r.monto)}</div>
                    </label>
                  ))}
                </div>
              </>
        ...
        <DialogFooter>
          {stage === "form" ? (
            <>
              ...
              <Button onClick={() => setStage("preview")} disabled={!edificioId || !concepto || !mes || !vencimiento} className="bg-[#173B7A] hover:bg-[#0f2659]">Previsualizar</Button>
            </>
          ) : (
            <>
              <Button variant="ghost" onClick={() => setStage("form")}>Volver</Button>
              <Button onClick={confirm} disabled={selectedIds.length === 0 || mut.isPending} className="bg-[#173B7A] hover:bg-[#0f2659]">
