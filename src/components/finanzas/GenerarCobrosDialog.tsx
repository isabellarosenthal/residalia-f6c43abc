  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) { setStage("form"); setExcluded(new Set()); } onOpenChange(v); }}>
      <DialogContent className="sm:max-w-[640px] max-h-[85vh] overflow-y-auto">
        <DialogHeader><DialogTitle className="font-display text-xl text-[#0a1e3f]">Generar cobros mensuales</DialogTitle></DialogHeader>

        {stage === "form" && (
          <div className="space-y-3">
            <p className="text-sm text-[#6b7a99]">Generaremos un cobro por cada unidad usando su mantenimiento mensual o la cuota base del edificio.</p>
        ...
        {stage === "preview" && (
          <div className="space-y-3">
            {isLoading && <div className="py-8 text-center text-[#6b7a99]">Calculando…</div>}
            {!isLoading && (
              <>
                  ...
                  {rows.map((r) => (
                    <label key={r.unidad_id} className={`flex items-center gap-3 p-3 ${r.duplicado ? "opacity-60 bg-[#fafafa]" : "hover:bg-[#fbf6f3] cursor-pointer"}`}>
                      ...
                      <div className="flex-1">
                        <div className="text-sm font-medium text-[#0a1e3f]">Unidad #{r.unidad_numero}</div>
                      ...
                      <div className="text-sm font-semibold text-[#0a1e3f] tabular-nums">{fmtL(r.monto)}</div>
                    </label>
                  ))}
                </div>
              </>
