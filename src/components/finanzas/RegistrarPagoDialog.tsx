    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="grid grid-cols-3 gap-2 text-sm mb-3">
          ...
          <div className="p-2 rounded-lg bg-[#F8FAFC]"><div className="text-xs text-[#64748B]">Saldo</div><div className="font-bold text-[#2D3748]">{fmtL(saldo)}</div></div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 border-t border-[#E2E8F0] pt-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <button type="button" onClick={() => setMonto(String(saldo))} className="text-xs text-[#2D3748] hover:underline mt-1">Usar saldo {fmtL(saldo)}</button>
              </div>
              <div>
                <Label>Fecha</Label>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={reg.isPending} className="bg-[#2D3748] hover:bg-[#1F2937]">Registrar pago</Button>
            </DialogFooter>
          </form>
        )}
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {pagos.map((p) => (
                <div key={p.id} className="flex items-center justify-between bg-[#faf6f3] rounded-lg px-3 py-2 text-sm">
                  <div>
                    <div className="font-semibold text-[#2D3748]">{fmtL(p.monto)} <span className="text-xs text-[#64748B] font-normal">· {p.metodo}</span></div>
