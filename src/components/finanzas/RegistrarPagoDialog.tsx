    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="grid grid-cols-3 gap-2 text-sm mb-3">
          <div className="p-2 rounded-lg bg-[#eef2ff]"><div className="text-xs text-[#8b8bb5]">Total</div><div className="font-bold">{fmtL(cobro.monto)}</div></div>
          <div className="p-2 rounded-lg bg-[#eef2ff]"><div className="text-xs text-[#8b8bb5]">Abonado</div><div className="font-bold text-[#166534]">{fmtL(abonado)}</div></div>
          <div className="p-2 rounded-lg bg-[#eef2ff]"><div className="text-xs text-[#8b8bb5]">Saldo</div><div className="font-bold text-[#818cf8]">{fmtL(saldo)}</div></div>
        </div>

        {saldo > 0 && (
          <form onSubmit={handleSubmit} className="space-y-3 border-t border-[#e0e7ff] pt-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Monto *</Label>
                <Input type="number" step="0.01" min="0.01" max={saldo} value={monto} onChange={(e) => setMonto(e.target.value)} required />
                <button type="button" onClick={() => setMonto(String(saldo))} className="text-xs text-[#818cf8] hover:underline mt-1">Usar saldo {fmtL(saldo)}</button>
              </div>
              <div>
                <Label>Fecha</Label>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={reg.isPending} className="bg-[#818cf8] hover:bg-[#6366f1]">Registrar pago</Button>
            </DialogFooter>
          </form>
        )}

        {pagos.length > 0 && (
          <div className="border-t border-[#e0e7ff] pt-3 mt-3">
            <div className="text-xs uppercase tracking-wider text-[#8b8bb5] mb-2">Historial de pagos</div>
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {pagos.map((p) => (
                <div key={p.id} className="flex items-center justify-between bg-[#faf6f3] rounded-lg px-3 py-2 text-sm">
                  <div>
                    <div className="font-semibold text-[#1e1b4b]">{fmtL(p.monto)} <span className="text-xs text-[#8b8bb5] font-normal">· {p.metodo}</span></div>
                    <div className="text-xs text-[#8b8bb5]">{fmtDate(p.fecha)}{p.referencia ? ` · ${p.referencia}` : ""}</div>
                  </div>
