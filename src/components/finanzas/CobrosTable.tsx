  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <label className="flex items-center gap-2 text-sm text-[#2D3748] cursor-pointer select-none px-2">
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-[#F8FAFC] hover:bg-[#F8FAFC]">
              <TableHead className="text-[#2D3748] font-semibold">Concepto</TableHead>
              <TableHead className="text-[#2D3748] font-semibold">Unidad · Residente</TableHead>
              <TableHead className="text-[#2D3748] font-semibold text-right">Monto</TableHead>
              <TableHead className="text-[#2D3748] font-semibold text-right">Abonado</TableHead>
              <TableHead className="text-[#2D3748] font-semibold text-right">Saldo</TableHead>
              <TableHead className="text-[#2D3748] font-semibold">Vence</TableHead>
          ...
          <TableBody>
            ...
            {filtered.map((c) => {
              ...
              return (
                <TableRow key={c.id}>
                  <TableCell><div className="font-medium text-[#2D3748]">{c.concepto}</div>{c.recibo_numero && <div className="text-xs text-[#64748B]">{c.recibo_numero}</div>}</TableCell>
                  <TableCell className="text-sm">
                    <div className="text-[#2D3748]">{c.unidad_id ? `#${uniMap.get(c.unidad_id) ?? "—"}` : "—"}</div>
                    <div className="text-xs text-[#64748B]">{c.residente_id ? resMap.get(c.residente_id) ?? "—" : "—"}</div>
                  </TableCell>
                  <TableCell className="text-right font-semibold text-[#2D3748]">{fmtL(c.monto)}</TableCell>
                  <TableCell className="text-right text-sm text-[#166534]">{abonado > 0 ? fmtL(abonado) : "—"}</TableCell>
                  <TableCell className="text-right font-semibold text-[#2D3748]">{saldo > 0 ? fmtL(saldo) : "—"}</TableCell>
                  ...
                  <TableCell className="text-right">
                    {c.estado === "pagado" && (
                      <Link to="/recibo/$cobroId" params={{ cobroId: c.id }} target="_blank">
                        <Button size="sm" variant="ghost" title="Imprimir recibo" className="h-8 w-8 p-0 text-[#2D3748]"><Printer className="w-4 h-4" /></Button>
                      </Link>
                    )}
