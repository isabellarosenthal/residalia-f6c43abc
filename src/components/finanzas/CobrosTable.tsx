        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-[#f5ede8] hover:bg-[#f5ede8]">
              ...
              <TableHead className="text-[#2d1200] font-semibold text-right">Abonado</TableHead>
              <TableHead className="text-[#2d1200] font-semibold text-right">Saldo</TableHead>
          ...
          <TableBody>
            {isLoading && <TableRow><TableCell colSpan={8} className="py-10 text-center text-[#9a7060]">Cargando…</TableCell></TableRow>}
            {!isLoading && filtered.length === 0 && <TableRow><TableCell colSpan={8} className="py-10 text-center text-[#9a7060]">Sin cobros para los filtros.</TableCell></TableRow>}
            {filtered.map((c) => {
              const abonado = abonadoMap.get(c.id) ?? 0;
              const saldo = Math.max(0, Number(c.monto) - abonado);
              return (
                <TableRow key={c.id}>
                  <TableCell><div className="font-medium text-[#2d1200]">{c.concepto}</div>{c.recibo_numero && <div className="text-xs text-[#9a7060]">{c.recibo_numero}</div>}</TableCell>
                  ...
                  <TableCell className="text-right font-semibold text-[#2d1200]">{fmtL(c.monto)}</TableCell>
                  <TableCell className="text-right text-sm text-[#2d6a2d]">{abonado > 0 ? fmtL(abonado) : "—"}</TableCell>
                  <TableCell className="text-right font-semibold text-[#c94f0c]">{saldo > 0 ? fmtL(saldo) : "—"}</TableCell>
                  ...
                  <TableCell className="text-right">
                    ...
                    <Button size="sm" variant="ghost" title={saldo > 0 ? "Registrar pago" : "Ver pagos"} onClick={() => setPagoCobro(c)} className="h-8 w-8 p-0 text-[#2d6a2d]"><DollarSign className="w-4 h-4" /></Button>
