  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <label className="flex items-center gap-2 text-sm text-[#1e1b4b] cursor-pointer select-none px-2">
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-[#eef2ff] hover:bg-[#eef2ff]">
              <TableHead className="text-[#1e1b4b] font-semibold">Concepto</TableHead>
              ...
              <TableHead className="text-[#1e1b4b] font-semibold text-right">Abonado</TableHead>
              <TableHead className="text-[#1e1b4b] font-semibold text-right">Saldo</TableHead>
              <TableHead className="text-[#1e1b4b] font-semibold">Vence</TableHead>
          ...
          <TableBody>
            {isLoading && <TableRow><TableCell colSpan={8} className="py-10 text-center text-[#8b8bb5]">Cargando…</TableCell></TableRow>}
            {!isLoading && filtered.length === 0 && <TableRow><TableCell colSpan={8} className="py-10 text-center text-[#8b8bb5]">Sin cobros para los filtros.</TableCell></TableRow>}
            {filtered.map((c) => {
              const abonado = abonadoMap.get(c.id) ?? 0;
              const saldo = Math.max(0, Number(c.monto) - abonado);
              return (
                <TableRow key={c.id}>
                  <TableCell><div className="font-medium text-[#1e1b4b]">{c.concepto}</div>{c.recibo_numero && <div className="text-xs text-[#8b8bb5]">{c.recibo_numero}</div>}</TableCell>
                  <TableCell className="text-sm">
                    <div className="text-[#1e1b4b]">{c.unidad_id ? `#${uniMap.get(c.unidad_id) ?? "—"}` : "—"}</div>
                    <div className="text-xs text-[#8b8bb5]">{c.residente_id ? resMap.get(c.residente_id) ?? "—" : "—"}</div>
                  </TableCell>
                  <TableCell className="text-right font-semibold text-[#1e1b4b]">{fmtL(c.monto)}</TableCell>
                  <TableCell className="text-right text-sm text-[#166534]">{abonado > 0 ? fmtL(abonado) : "—"}</TableCell>
                  <TableCell className="text-right font-semibold text-[#818cf8]">{saldo > 0 ? fmtL(saldo) : "—"}</TableCell>
                  ...
                  <TableCell className="text-right">
                    {c.estado === "pagado" && (
                      <Link to="/recibo/$cobroId" params={{ cobroId: c.id }} target="_blank">
                        <Button size="sm" variant="ghost" title="Imprimir recibo" className="h-8 w-8 p-0 text-[#1e1b4b]"><Printer className="w-4 h-4" /></Button>
                      </Link>
                    )}
