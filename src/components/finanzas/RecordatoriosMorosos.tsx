    ...
    <div className="space-y-5">
      <Card className="p-5">
        <div className="flex items-start gap-3 mb-3">
          <BellRing className="w-5 h-5 text-[#c94f0c] mt-0.5" />
          <div>
            <h3 className="font-display font-bold text-[#2d1200]">Recordatorios a morosos</h3>
            <p className="text-sm text-[#9a7060]">Edita la plantilla y envía por WhatsApp o email. Variables: <code>{`{{nombre}}`}</code>, <code>{`{{total}}`}</code>, <code>{`{{unidad}}`}</code>, <code>{`{{edificio}}`}</code>, <code>{`{{detalle}}`}</code>.</p>
          </div>
        </div>
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-display font-bold text-[#2d1200]">{morosos.length} residente{morosos.length === 1 ? "" : "s"} con saldo</h3>
        </div>
        <div className="space-y-3">
          {morosos.length === 0 && <div className="text-center text-[#9a7060] py-8">Sin morosos en este edificio 🎉</div>}
          {morosos.map((m) => {
            const wa = waLink(m);
            const ml = mailLink(m);
            return (
              <div key={m.residenteId} className="border border-[#e8ddd8] rounded-xl p-4 flex flex-wrap gap-4 items-start">
                <div className="flex-1 min-w-[200px]">
                  <div className="font-semibold text-[#2d1200]">{m.nombre} <span className="text-xs text-[#9a7060] font-normal">· Unidad {m.unidad}</span></div>
                  <div className="text-xs text-[#9a7060] mt-0.5">{m.cobros.length} cobro{m.cobros.length === 1 ? "" : "s"} · máx {m.diasMax > 0 ? `${m.diasMax} días vencido` : "vigente"}</div>
                  <div className="text-xs text-[#9a7060] mt-1">{m.email ?? "sin email"} · {m.whatsapp ?? "sin teléfono"}</div>
                </div>
                <div className="text-right">
                  <div className="font-display font-extrabold text-[#c0392b] text-xl">{fmtL(m.total)}</div>
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  <Button size="sm" className="bg-[#c94f0c] hover:bg-[#a33d08]" disabled={!ml} asChild={!!ml}>
