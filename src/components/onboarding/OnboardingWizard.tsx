  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && close()}>
      <DialogContent className="max-w-2xl bg-[#ffffff] border-[#E2E8F0]">
        <button onClick={close} className="absolute right-4 top-4 text-[#64748B] hover:text-[#2D3748]"><X className="w-4 h-4" /></button>
        ...
        <h2 className="font-display font-extrabold text-2xl text-[#2D3748]">Configura tu primer edificio en 4 pasos</h2>
              ...
              <div key={s.label} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center gap-1">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors font-display font-extrabold ${
                    done ? "bg-[#166534] text-white" : active ? "bg-[#D97757] text-[#2D3748]" : "bg-[#F8FAFC] text-[#64748B]"
                  }`}>
                    {done ? <Check className="w-5 h-5" /> : <Icon className="w-5 h-5" />}
                  </div>
                  <span className={`text-xs ${active ? "text-[#2D3748] font-semibold" : "text-[#64748B]"}`}>{s.label}</span>
                </div>
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 space-y-3">
          {step === 0 && (
            <>
              <h3 className="font-display font-bold text-[#2D3748]">Datos del edificio</h3>
          ...
          {step === 1 && (
            <>
              <h3 className="font-display font-bold text-[#2D3748]">Genera tus unidades</h3>
          ...
          {step === 2 && (
            <>
              <h3 className="font-display font-bold text-[#2D3748]">Registra tu primer residente</h3>
          ...
          {step === 3 && (
            <>
              <h3 className="font-display font-bold text-[#2D3748]">Genera los cobros del mes</h3>
        ...
        <div className="flex items-center justify-between gap-2 pt-4">
          ...
          <div className="flex gap-2">
            ...
            {step === 0 && <Button className="bg-[#2D3748] hover:bg-[#1F2937]" onClick={handleStep1} disabled={saveEdif.isPending}>Continuar <ArrowRight className="w-4 h-4 ml-1" /></Button>}
            {step === 1 && <Button className="bg-[#2D3748] hover:bg-[#1F2937]" onClick={handleStep2} disabled={bulkUnidades.isPending}>Generar {pisos * porPiso} unidades <ArrowRight className="w-4 h-4 ml-1" /></Button>}
            {step === 2 && <Button className="bg-[#2D3748] hover:bg-[#1F2937]" onClick={handleStep3} disabled={saveRes.isPending}>Continuar <ArrowRight className="w-4 h-4 ml-1" /></Button>}
