...
export function PropiedadCard({
  ...
  return (
    <div className="group bg-white border border-[#e8ddd8] rounded-2xl overflow-hidden hover:shadow-lg hover:border-[#c94f0c]/40 transition-all">
      <div className="relative aspect-[4/3] bg-gradient-to-br from-[#f5ede8] to-[#e8ddd8] overflow-hidden">
        {cover ? (
        ...
        {unidad.precio_negociable && (
          <div className="absolute top-3 right-3 bg-white/95 backdrop-blur text-[10px] font-semibold px-2 py-1 rounded-full text-[#2d1200]">
            Negociable
          </div>
        )}
        <button
          onClick={() => onEdit(unidad)}
          className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition bg-white/95 backdrop-blur p-2 rounded-full text-[#2d1200] hover:bg-[#c94f0c] hover:text-white"
          aria-label="Editar"
        >
      ...
      <div className="p-4 space-y-3">
        <div>
          <div className="flex items-start justify-between gap-2">
            <h3 className="font-display font-bold text-[#2d1200] text-lg leading-tight">
              {unidad.tipo ? unidad.tipo[0].toUpperCase() + unidad.tipo.slice(1) : "Unidad"} #{unidad.numero}
            </h3>
            {unidad.piso != null && <span className="text-xs text-[#9a7060] mt-1">Piso {unidad.piso}</span>}
          </div>
          {edificio && (
            <p className="text-xs text-[#9a7060] flex items-center gap-1 mt-0.5">
        ...
        <div className="pt-2 border-t border-[#f0e6e0] space-y-1">
          {muestraVenta && (
            <div className="flex items-center justify-between">
              <span className="text-[10px] uppercase tracking-wider text-[#9a7060] font-semibold">Venta</span>
              <span className="font-display font-bold text-[#c94f0c]">{fmt(unidad.precio_venta, moneda)}</span>
            </div>
          )}
          {muestraRenta && (
            <div className="flex items-center justify-between">
              <span className="text-[10px] uppercase tracking-wider text-[#9a7060] font-semibold">Renta/mes</span>
              <span className="font-display font-bold text-[#2d1200]">{fmt(unidad.precio_renta, moneda)}</span>
            </div>
          )}
          {!muestraVenta && !muestraRenta && (
            <div className="flex items-center gap-1 text-xs text-[#9a7060]"><Tag className="w-3 h-3" />Sin publicar comercialmente</div>
          )}
        </div>
      </div>
    </div>
