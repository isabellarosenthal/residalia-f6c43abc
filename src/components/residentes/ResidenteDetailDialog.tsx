  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[760px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display text-xl text-[#2d1200]">
...
function DetailBody({ residente }: { residente: Residente }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 items-center text-sm text-[#4a2800] bg-[#f5ede8] rounded-lg p-3">
        ...
        <span className="text-[#9a7060]">{edif?.nombre ?? "—"} {uni ? `· #${uni.numero}` : ""}</span>
...
function PersonasTab({ residenteId }: { residenteId: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_160px_auto] gap-2 items-end border border-[#e8ddd8] rounded-lg p-3 bg-[#fdfaf7]">
        ...
        <Button onClick={add} disabled={!nombre.trim() || save.isPending} className="bg-[#c94f0c] hover:bg-[#a33d08]">
          <Plus className="w-4 h-4 mr-1" />Agregar
        </Button>
      </div>

      {isLoading ? <p className="text-sm text-[#9a7060]">Cargando…</p> : personas.length === 0 ? (
        <p className="text-sm text-[#9a7060] text-center py-4">Sin personas autorizadas.</p>
      ) : (
        <div className="space-y-1">
          {personas.map((p) => (
            <div key={p.id} className="flex items-center justify-between border border-[#e8ddd8] rounded-lg px-3 py-2">
              <div>
                <div className="text-sm font-medium text-[#2d1200]">{p.nombre}</div>
                <div className="text-xs text-[#9a7060]">{p.relacion ?? "—"} · {p.tipo_acceso ?? "permanente"}</div>
              </div>
              <Button size="sm" variant="ghost" onClick={() => del.mutate({ id: p.id, residenteId })} className="h-8 w-8 p-0 text-[#c0392b]"><Trash2 className="w-4 h-4" /></Button>
            </div>
          ))}
        </div>
      )}
...
function VehiculosTab({ residenteId }: { residenteId: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 md:grid-cols-[120px_1fr_1fr_120px_auto] gap-2 items-end border border-[#e8ddd8] rounded-lg p-3 bg-[#fdfaf7]">
        ...
        <Button onClick={add} disabled={!placa.trim() || save.isPending} className="bg-[#c94f0c] hover:bg-[#a33d08]"><Plus className="w-4 h-4 mr-1" />Agregar</Button>
      </div>

      {isLoading ? <p className="text-sm text-[#9a7060]">Cargando…</p> : vehiculos.length === 0 ? (
        <p className="text-sm text-[#9a7060] text-center py-4">Sin vehículos registrados.</p>
      ) : (
        <div className="space-y-1">
          {vehiculos.map((v) => (
            <div key={v.id} className="flex items-center justify-between border border-[#e8ddd8] rounded-lg px-3 py-2">
              <div className="flex items-center gap-3">
                <Badge variant="neutral">{v.placa}</Badge>
                <div className="text-sm text-[#2d1200]">
                  {[v.marca, v.modelo].filter(Boolean).join(" ") || "—"}
                  {v.color && <span className="text-xs text-[#9a7060]"> · {v.color}</span>}
                </div>
              </div>
              <Button size="sm" variant="ghost" onClick={() => del.mutate({ id: v.id, residenteId })} className="h-8 w-8 p-0 text-[#c0392b]"><Trash2 className="w-4 h-4" /></Button>
            </div>
          ))}
        </div>
      )}
...
function CuentaTab({ residenteId }: { residenteId: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {cobros.length === 0 ? (
        <p className="text-sm text-[#9a7060] text-center py-4">Sin cobros asociados.</p>
      ) : (
          ...
          {cobros.map((c) => {
            ...
            return (
              <div key={c.id} className="flex items-center justify-between border border-[#e8ddd8] rounded-lg px-3 py-2 text-sm">
                <div>
                  <div className="font-medium text-[#2d1200]">{c.concepto}</div>
