      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-[#fffdf5] hover:bg-[#fffdf5]">
            <TableHead className="text-[#0a1e3f] font-semibold">Residente</TableHead>
            <TableHead className="text-[#0a1e3f] font-semibold">Tipo</TableHead>
            <TableHead className="text-[#0a1e3f] font-semibold">Edificio · Unidad</TableHead>
            <TableHead className="text-[#0a1e3f] font-semibold">Contacto</TableHead>
            <TableHead className="text-[#0a1e3f] font-semibold">Ingreso</TableHead>
        ...
        <TableBody>
          ...
          {!isLoading && filtered.length === 0 && <TableRow><TableCell colSpan={7} className="py-10 text-center text-[#6b7a99]">Sin residentes para los filtros.</TableCell></TableRow>}
          {filtered.map((r) => (
            <TableRow key={r.id}>
              <TableCell>
                <div className="font-semibold text-[#0a1e3f]">{r.nombre} {r.apellido}</div>
                {r.dni && <div className="text-xs text-[#6b7a99]">DNI {r.dni}</div>}
              </TableCell>
              <TableCell>
                <Badge variant={r.tipo === "propietario" ? "venta" : "renta"}>{r.tipo}</Badge>
              </TableCell>
              <TableCell className="text-sm">
                <div className="text-[#0a1e3f]">{edifMap.get(r.condominio_id) ?? "—"}</div>
                <div className="text-xs text-[#6b7a99]">{r.unidad_id ? `Unidad #${uniMap.get(r.unidad_id) ?? "—"}` : "Sin unidad"}</div>
              </TableCell>
              <TableCell className="text-sm">
                {r.telefono && <div className="flex items-center gap-1 text-[#13294b]"><Phone className="w-3 h-3" />{r.telefono}</div>}
                {r.email && <div className="flex items-center gap-1 text-xs text-[#6b7a99]"><Mail className="w-3 h-3" />{r.email}</div>}
                {!r.telefono && !r.email && <span className="text-[#6b7a99]">—</span>}
              </TableCell>
              <TableCell className="text-sm text-[#13294b]">{fmtDate(r.fecha_ingreso)}</TableCell>
