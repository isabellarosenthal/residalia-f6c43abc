      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-[#f5ede8] hover:bg-[#f5ede8]">
            <TableHead className="text-[#2d1200] font-semibold">Residente</TableHead>
            <TableHead className="text-[#2d1200] font-semibold">Tipo</TableHead>
            <TableHead className="text-[#2d1200] font-semibold">Edificio · Unidad</TableHead>
            <TableHead className="text-[#2d1200] font-semibold">Contacto</TableHead>
            <TableHead className="text-[#2d1200] font-semibold">Ingreso</TableHead>
        ...
        <TableBody>
          ...
          {!isLoading && filtered.length === 0 && <TableRow><TableCell colSpan={7} className="py-10 text-center text-[#9a7060]">Sin residentes para los filtros.</TableCell></TableRow>}
          {filtered.map((r) => (
            <TableRow key={r.id}>
              <TableCell>
                <div className="font-semibold text-[#2d1200]">{r.nombre} {r.apellido}</div>
                {r.dni && <div className="text-xs text-[#9a7060]">DNI {r.dni}</div>}
              </TableCell>
              <TableCell>
                <Badge variant={r.tipo === "propietario" ? "venta" : "renta"}>{r.tipo}</Badge>
              </TableCell>
              <TableCell className="text-sm">
                <div className="text-[#2d1200]">{edifMap.get(r.condominio_id) ?? "—"}</div>
                <div className="text-xs text-[#9a7060]">{r.unidad_id ? `Unidad #${uniMap.get(r.unidad_id) ?? "—"}` : "Sin unidad"}</div>
              </TableCell>
              <TableCell className="text-sm">
                {r.telefono && <div className="flex items-center gap-1 text-[#4a2800]"><Phone className="w-3 h-3" />{r.telefono}</div>}
                {r.email && <div className="flex items-center gap-1 text-xs text-[#9a7060]"><Mail className="w-3 h-3" />{r.email}</div>}
                {!r.telefono && !r.email && <span className="text-[#9a7060]">—</span>}
              </TableCell>
