...
export function Card({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={`bg-white border border-[#e8ecf3] rounded-2xl shadow-sm ${className}`}>{children}</div>;
}
  ...
  return (
    <Card className="p-5">
      <div className="flex items-start justify-between">
        ...
        <div className={`w-9 h-9 rounded-xl bg-[#fffdf5] flex items-center justify-center ${accentMap[accent]}`}>{icon}</div>
      </div>
      <div className={`mt-3 font-display font-extrabold text-3xl ${accentMap[accent]}`}>{value}</div>
      {sub && <div className="mt-1 text-xs text-[#6b7a99]">{sub}</div>}
    </Card>
  );
}
