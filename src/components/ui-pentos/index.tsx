...
export function Badge({ children, variant = "neutral" }: { children: ReactNode; variant?: "success" | "danger" | "warning" | "neutral" | "venta" | "renta" | "reservada" }) {
  const map = {
    ...
    warning: "bg-[#FAF1DC] text-[#173B7A]",
    neutral: "bg-[#ede8e5] text-[#64748B]",
    venta: "bg-[#173B7A] text-white",
    renta: "bg-[#173B7A] text-[#F8FAFC]",
...
export function EmptyState({ icon, title, hint, action }: { icon: ReactNode; title: string; hint?: string; action?: ReactNode }) {
  return (
    <Card className="p-10 text-center">
      <div className="w-14 h-14 rounded-2xl bg-[#FAF1DC] text-[#173B7A] flex items-center justify-center mx-auto mb-4">{icon}</div>
      <div className="font-display font-bold text-lg text-[#173B7A]">{title}</div>
