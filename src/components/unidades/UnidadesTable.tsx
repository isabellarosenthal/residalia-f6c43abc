    ...
    <div className="space-y-4">
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 bg-[#fff5ec] border border-[#f3d6bd] rounded-xl px-3 py-2">
          <span className="text-sm font-semibold text-[#4A154B]">{selected.size} seleccionada{selected.size > 1 ? "s" : ""}</span>
            ...
            {filtered.map((u) => (
              <TableRow key={u.id} data-state={selected.has(u.id) ? "selected" : undefined}>
                ...
                <TableCell className="text-sm">
                  <div className="text-[#4A154B]">{u.propietario_id ? residentesMap?.get(u.propietario_id) ?? "—" : "—"}</div>
