    ...
    <div className="space-y-4">
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 bg-[#fff5ec] border border-[#f3d6bd] rounded-xl px-3 py-2">
          <span className="text-sm font-semibold text-[#4F46E5]">{selected.size} seleccionada{selected.size > 1 ? "s" : ""}</span>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-[#F8FAFC] hover:bg-[#F8FAFC]">
              ...
              <TableHead className="text-[#4F46E5] font-semibold">Estado admin.</TableHead>
              <TableHead className="text-[#4F46E5] font-semibold">Estado comercial</TableHead>
              <TableHead className="text-[#4F46E5] font-semibold">Propietario / Inquilino</TableHead>
              <TableHead className="text-[#4F46E5] font-semibold text-right">Precios</TableHead>
            ...
            {filtered.map((u) => (
              <TableRow key={u.id} data-state={selected.has(u.id) ? "selected" : undefined}>
                ...
                <TableCell className="text-sm">
                  <div className="text-[#4F46E5]">{u.propietario_id ? residentesMap?.get(u.propietario_id) ?? "—" : "—"}</div>
                  <div className="text-xs text-[#64748B]">{u.inquilino_id ? `Inq: ${residentesMap?.get(u.inquilino_id) ?? "—"}` : ""}</div>
                </TableCell>
                <TableCell className="text-right">
                  {u.precio_venta && <div className="text-sm font-semibold text-[#4F46E5]">{fmtMoney(u.precio_venta, (u as any).moneda)}</div>}
