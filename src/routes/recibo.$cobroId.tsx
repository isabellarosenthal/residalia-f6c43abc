  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] py-8 print:bg-white print:py-0">
      ...
      <div className="max-w-[720px] mx-auto mb-4 flex justify-between items-center no-print">
        ...
        <Button onClick={() => window.print()} className="bg-[#4F46E5] hover:bg-[#4338CA]"><Printer className="w-4 h-4 mr-2" />Imprimir</Button>
      </div>
      ...
      <div className="max-w-[720px] mx-auto bg-white border border-[#E2E8F0] rounded-2xl print:border-0 print:rounded-none shadow-sm p-10">
        ...
        <section className="grid grid-cols-2 gap-6 py-6 text-sm">
          <div>
            ...
            <div className="font-medium text-[#4F46E5]">{residente ? `${residente.nombre} ${residente.apellido ?? ""}`.trim() : "—"}</div>
          ...
          <div>
            ...
            <div className="font-medium text-[#4F46E5]">{unidad ? `#${unidad.numero}${unidad.piso != null ? ` · piso ${unidad.piso}` : ""}` : "—"}</div>
            <div className="text-xs text-[#64748B] mt-2 uppercase tracking-wider">Método de pago</div>
            <div className="font-medium text-[#4F46E5] capitalize">{cobro.metodo_pago ?? "—"}</div>
          </div>
        </section>
        ...
        <section className="border border-[#E2E8F0] rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-[#F8FAFC]">
              <tr>
                <th className="text-left p-3 font-semibold text-[#4F46E5]">Concepto</th>
                <th className="text-right p-3 font-semibold text-[#4F46E5]">Monto</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="p-3 text-[#4F46E5]">{cobro.concepto}<div className="text-xs text-[#64748B]">Vencía: {fmtDate(cobro.fecha_vencimiento)}</div></td>
                <td className="p-3 text-right font-semibold text-[#4F46E5]">{fmtL(Number(cobro.monto))}</td>
              </tr>
            </tbody>
