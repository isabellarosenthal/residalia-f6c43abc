...
function ReciboPage() {
  ...
  if (isLoading) return <div className="p-10 text-center text-[#64748B]">Cargando recibo…</div>;
  if (!data?.cobro) return <div className="p-10 text-center text-[#64748B]">Recibo no encontrado.</div>;
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] py-8 print:bg-white print:py-0">
      ...
      <div className="max-w-[720px] mx-auto mb-4 flex justify-between items-center no-print">
        ...
        <Button onClick={() => window.print()} className="bg-[#2D3748] hover:bg-[#1F2937]"><Printer className="w-4 h-4 mr-2" />Imprimir</Button>
      </div>
      ...
      <div className="max-w-[720px] mx-auto bg-white border border-[#E2E8F0] rounded-2xl print:border-0 print:rounded-none shadow-sm p-10">
        <header className="flex items-start justify-between pb-6 border-b border-[#E2E8F0]">
          <div className="flex items-center gap-4">
            {edificio?.logo_url
              ? <img src={edificio.logo_url} alt="" className="w-14 h-14 rounded-xl object-cover" />
              : <div className="w-14 h-14 rounded-xl bg-[#2D3748] text-white flex items-center justify-center font-display font-extrabold text-lg">{edificio?.nombre?.[0] ?? "C"}</div>}
            <div>
              <div className="font-display font-extrabold text-xl text-[#2D3748]">{edificio?.nombre ?? "—"}</div>
          ...
          <div className="text-right">
            ...
            <div className="font-display font-bold text-lg text-[#2D3748]">{cobro.recibo_numero ?? "—"}</div>
        ...
        <section className="grid grid-cols-2 gap-6 py-6 text-sm">
          <div>
            ...
            <div className="font-medium text-[#2D3748]">{residente ? `${residente.nombre} ${residente.apellido ?? ""}`.trim() : "—"}</div>
          ...
          <div>
            ...
            <div className="font-medium text-[#2D3748]">{unidad ? `#${unidad.numero}${unidad.piso != null ? ` · piso ${unidad.piso}` : ""}` : "—"}</div>
            <div className="text-xs text-[#64748B] mt-2 uppercase tracking-wider">Método de pago</div>
            <div className="font-medium text-[#2D3748] capitalize">{cobro.metodo_pago ?? "—"}</div>
          </div>
        </section>

        <section className="border border-[#E2E8F0] rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-[#F8FAFC]">
              <tr>
                <th className="text-left p-3 font-semibold text-[#2D3748]">Concepto</th>
                <th className="text-right p-3 font-semibold text-[#2D3748]">Monto</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="p-3 text-[#2D3748]">{cobro.concepto}<div className="text-xs text-[#64748B]">Vencía: {fmtDate(cobro.fecha_vencimiento)}</div></td>
                <td className="p-3 text-right font-semibold text-[#2D3748]">{fmtL(Number(cobro.monto))}</td>
              </tr>
            </tbody>
            <tfoot className="bg-[#fbf6f3]">
              <tr>
                <td className="p-3 text-right font-semibold text-[#2D3748]">Total pagado</td>
                <td className="p-3 text-right font-display font-extrabold text-xl text-[#2D3748]">{fmtL(Number(cobro.monto))}</td>
              </tr>
            </tfoot>
          </table>
        </section>
        ...
        {cobro.notas && (
          <section className="mt-5 text-xs text-[#64748B]">
            <div className="uppercase tracking-wider mb-1">Notas</div>
            <div>{cobro.notas}</div>
          </section>
        )}

        <footer className="mt-8 pt-6 border-t border-[#E2E8F0] text-center text-xs text-[#64748B]">
