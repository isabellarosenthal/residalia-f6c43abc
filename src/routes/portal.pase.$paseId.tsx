...
${validar}`;
  ...
  return (
    <div className="space-y-4">
      <Link to="/portal" className="inline-flex items-center text-sm text-[#64748B] hover:text-[#173B7A]"><ArrowLeft className="w-4 h-4 mr-1" />Volver</Link>
      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 text-center">
        ...
        <div className="font-display font-extrabold text-2xl text-[#173B7A] mb-5">{pase.visitante_nombre}</div>

        <div className="flex justify-center mb-4"><QRCanvas value={validar} size={240} /></div>

        <div className="bg-[#ffffff] border border-dashed border-[#D9A441] rounded-xl p-3 mb-4">
          <div className="text-[11px] text-[#64748B] mb-1">Código</div>
          <div className="font-mono text-lg font-bold tracking-widest text-[#173B7A] break-all">{code}</div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          ...
          <button onClick={share} className="flex items-center justify-center gap-1.5 bg-[#173B7A] hover:bg-[#0f2659] text-white text-sm py-2.5 rounded-xl">
            <Share2 className="w-4 h-4" />Compartir
          </button>
          <button onClick={copy} className="flex items-center justify-center gap-1.5 border border-[#EBC988] text-[#173B7A] text-sm py-2.5 rounded-xl">
