...
function GuardiaValidar() {
  ...
  return (
    <div className="space-y-5">
      <form onSubmit={(e) => { e.preventDefault(); buscar(); }} className="flex gap-2 bg-[#4A154B] border border-[#350d36] rounded-2xl p-3">
        ...
        <button type="submit" disabled={!codigo.trim() || validar.isPending} className="bg-[#4A154B] hover:bg-[#e6c200] text-white px-4 py-2 rounded-lg inline-flex items-center gap-1 disabled:opacity-50">
          <Search className="w-4 h-4" />Buscar
        </button>
      </form>

      {notFound && <div className="bg-[#7a2a10] border border-[#4A154B] text-white rounded-2xl p-4 text-sm">No se encontró el pase.</div>}

      {acceso && estado && (
        <div className="bg-white text-[#4A154B] rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div><div className="text-xs text-[#64748B]">Edificio</div><div>{edif?.nombre ?? "—"}</div></div>
            <div><div className="text-xs text-[#64748B]">Unidad</div><div>{uni ? `#${uni.numero}` : "—"}</div></div>
            <div><div className="text-xs text-[#64748B]">Tipo</div><div className="capitalize">{acceso.tipo ?? "—"}</div></div>
            <div><div className="text-xs text-[#64748B]">Usos</div><div>{acceso.usos_actuales ?? 0} / {acceso.usos_maximos ?? 1}</div></div>
            <div><div className="text-xs text-[#64748B]">Entrada</div><div>{fmtDT(acceso.fecha_entrada)}</div></div>
            <div><div className="text-xs text-[#64748B]">Salida</div><div>{fmtDT(acceso.fecha_salida)}</div></div>
          </div>
          <div className="flex flex-wrap gap-2 pt-2 border-t border-[#f0e6e0]">
            {estado.tone === "success" && (
              <button onClick={async () => setAcceso(await registrarUso.mutateAsync(acceso))} disabled={registrarUso.isPending}
                className="bg-[#166534] hover:bg-[#1f4d1f] text-white px-4 py-2 rounded-lg inline-flex items-center gap-1"><LogIn className="w-4 h-4" />Autorizar entrada</button>
            )}
            {!acceso.fecha_salida && (
              <button onClick={async () => setAcceso(await marcarSalida.mutateAsync(acceso.id))} disabled={marcarSalida.isPending}
                className="border border-[#E8E8E8] text-[#4A154B] px-4 py-2 rounded-lg inline-flex items-center gap-1"><LogOut className="w-4 h-4" />Registrar salida</button>
            )}
            <button onClick={() => { setAcceso(null); setCodigo(""); }} className="text-[#64748B] hover:text-[#4A154B] px-3 py-2">Limpiar</button>
          </div>
        </div>
      )}
    </div>
