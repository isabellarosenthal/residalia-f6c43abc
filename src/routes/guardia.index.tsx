...
function GuardiaValidar() {
  ...
  return (
    <div className="space-y-5">
      <form onSubmit={(e) => { e.preventDefault(); buscar(); }} className="flex gap-2 bg-[#374151] border border-[#1F2937] rounded-2xl p-3">
      ...
      {acceso && estado && (
        <div className="bg-white text-[#374151] rounded-2xl p-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t border-[#f0e6e0]">
            ...
            {!acceso.fecha_salida && (
              <button onClick={async () => setAcceso(await marcarSalida.mutateAsync(acceso.id))} disabled={marcarSalida.isPending}
                className="border border-[#EBC988] text-[#374151] px-4 py-2 rounded-lg inline-flex items-center gap-1"><LogOut className="w-4 h-4" />Registrar salida</button>
            )}
