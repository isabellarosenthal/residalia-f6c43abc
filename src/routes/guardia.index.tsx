...
function GuardiaValidar() {
  ...
  return (
    <div className="space-y-5">
      <form onSubmit={(e) => { e.preventDefault(); buscar(); }} className="flex gap-2 bg-[#173B7A] border border-[#0f2659] rounded-2xl p-3">
        ...
        <button type="submit" disabled={!codigo.trim() || validar.isPending} className="bg-[#D9A441] hover:bg-[#e6c200] text-white px-4 py-2 rounded-lg inline-flex items-center gap-1 disabled:opacity-50">
          <Search className="w-4 h-4" />Buscar
        </button>
      </form>

      {notFound && <div className="bg-[#7a2a10] border border-[#D9A441] text-white rounded-2xl p-4 text-sm">No se encontró el pase.</div>}

      {acceso && estado && (
        <div className="bg-white text-[#173B7A] rounded-2xl p-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t border-[#f0e6e0]">
            ...
            {!acceso.fecha_salida && (
              <button onClick={async () => setAcceso(await marcarSalida.mutateAsync(acceso.id))} disabled={marcarSalida.isPending}
                className="border border-[#EBC988] text-[#173B7A] px-4 py-2 rounded-lg inline-flex items-center gap-1"><LogOut className="w-4 h-4" />Registrar salida</button>
            )}
            <button onClick={() => { setAcceso(null); setCodigo(""); }} className="text-[#64748B] hover:text-[#D9A441] px-3 py-2">Limpiar</button>
          </div>
        </div>
      )}
    </div>
