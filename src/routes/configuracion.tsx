      ...
      <div className="space-y-5 max-w-[1100px] mx-auto">
        ...
        <Tabs defaultValue="perfil">
          ...
          <TabsContent value="usuarios" className="pt-4">
            {isSuper ? <UsuariosTab /> : canManage ? <TenantUsuariosTab edificios={edificios} /> : <p className="text-sm text-[#6b7a99] p-4">No tienes edificios asignados.</p>}
          </TabsContent>
    ...
    <Card className="p-6 space-y-4">
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-[#6b7a99] mt-1">Cuántos Lempiras equivalen a 1 USD. Se usa para convertir precios entre monedas.</p>
        </div>
      </div>
...
function EdificiosTab() {
  ...
  return (
    <div className="space-y-3">
        ...
        <Card key={e.id} className="p-4">
          {editing === e.id ? (
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="font-display font-bold text-[#0a1e3f]">{e.nombre}</div>
                <div className="text-sm text-[#6b7a99]">Cuota base: {e.moneda} {Number(e.cuota_base ?? 0).toLocaleString()} · {e.total_unidades ?? 0} unidades</div>
              </div>
      ...
      {edificios.length === 0 && <p className="text-sm text-[#6b7a99]">No hay edificios. Créalos desde el módulo Edificios.</p>}
    </div>
  );
}
...
function UsuariosTab() {
  ...
  return (
    ...
    <Card className="p-5 space-y-4">
      <div className="border border-dashed border-[#e8ecf3] rounded-xl p-4 bg-[#ffffff]">
        <div className="text-sm font-medium text-[#0a1e3f] mb-2">Asignar rol por email</div>
        <p className="text-xs text-[#6b7a99] mb-3">El usuario debe haberse registrado primero en /login</p>
      ...
      {loading ? <p className="text-sm text-[#6b7a99]">Cargando…</p> : (
        <div className="divide-y divide-[#f0e6e0]">
          {rows.map(r => (
            <div key={r.id} className="py-3 flex items-center justify-between gap-3">
              <div>
                <div className="font-medium text-[#0a1e3f]">{r.full_name}</div>
                <div className="text-xs text-[#6b7a99]">{r.email}</div>
              </div>
...
function TenantUsuariosTab({ edificios }: { edificios: Edif[] }) {
  ...
  return (
    ...
    <Card className="p-5 space-y-4">
      ...
      <div className="border border-dashed border-[#e8ecf3] rounded-xl p-4 bg-[#ffffff]">
        <div className="text-sm font-medium text-[#0a1e3f] mb-2">Invitar staff o guardia</div>
        <p className="text-xs text-[#6b7a99] mb-3">El usuario debe registrarse primero en /login con su email. Luego asígnale aquí su rol y edificio.</p>
        <div className="flex flex-wrap gap-2">
          <Input className="flex-1 min-w-[220px]" placeholder="email@ejemplo.com" value={invite.email} onChange={(e) => setInvite({ ...invite, email: e.target.value })} />
          <Select value={invite.role} onValueChange={(v) => setInvite({ ...invite, role: v as AppRole })}>
            <SelectTrigger className="w-[200px]"><SelectValue /></SelectTrigger>
      ...
      <div className="text-xs text-[#6b7a99]">Para invitar residentes, usa el módulo <a className="underline" href="/residentes">Residentes</a>.</div>

      {loading ? <p className="text-sm text-[#6b7a99]">Cargando…</p> : members.length === 0 ? (
        <p className="text-sm text-[#6b7a99]">Sin miembros en este edificio.</p>
      ) : (
        <div className="divide-y divide-[#f0e6e0]">
          {members.map(m => (
            <div key={m.id} className="py-3 flex items-center justify-between gap-3">
              <div>
                <div className="font-medium text-[#0a1e3f]">{m.full_name}</div>
                <div className="text-xs text-[#6b7a99]">{m.email} · {m.role ?? "sin rol"}</div>
              </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-6 space-y-3 border-[#fbcfe8]">
        ...
        <p className="text-sm text-[#6b7a99]">Cerrar sesión en este dispositivo.</p>
        ...
        <div className="text-xs text-[#6b7a99] pt-2">ID de usuario: <code className="bg-[#fffdf5] px-2 py-0.5 rounded">{user?.id}</code></div>
      </Card>
    </div>
  );
}
  ...
  return (
    ...
    <Card className="p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="font-display font-bold text-lg text-[#0a1e3f]">Residentes</h3>
          <p className="text-sm text-[#6b7a99]">{isSuper ? "Todos los residentes de la plataforma" : "Residentes de tus edificios"} · Fecha de ingreso y registro en Altura Cloud</p>
        </div>
        <Input placeholder="Buscar..." value={q} onChange={(e) => setQ(e.target.value)} className="max-w-xs" />
      </div>

      {loading ? (
        <p className="text-sm text-[#6b7a99]">Cargando...</p>
      ) : filtered.length === 0 ? (
        <p className="text-sm text-[#6b7a99]">No hay residentes.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="text-left text-[#6b7a99] border-b border-[#e8ecf3]">
              <tr>
            ...
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id} className="border-b border-[#fffdf5]">
                  <td className="py-2 pr-3 font-medium text-[#0a1e3f]">{r.nombre} {r.apellido}</td>
                  <td className="py-2 pr-3 text-[#6b7a99]">{r.email ?? "—"}</td>
                  <td className="py-2 pr-3 text-[#6b7a99]">{r.condominio_nombre}</td>
                  <td className="py-2 pr-3 text-[#6b7a99]">{fmt(r.fecha_ingreso)}</td>
                  <td className="py-2 pr-3 text-[#6b7a99]">
...
function MiPlanTab() {
  ...
  if (isLoading) return <Card><div className="p-4 text-sm text-[#6b7a99]">Cargando plan...</div></Card>;
  if (!data) return <Card><div className="p-4 text-sm text-[#6b7a99]">Sin datos de plan</div></Card>;
    ...
    <div className="space-y-4">
      <Card>
        <div className="p-5 flex items-center justify-between flex-wrap gap-3">
          <div>
            <div className="text-xs uppercase tracking-wide text-[#6b7a99]">Plan actual</div>
            <div className="font-display font-extrabold text-2xl text-[#0a1e3f]">{data.plan.nombre}</div>
      ...
      <Card>
        <div className="p-5 space-y-2">
          <div className="flex items-center justify-between">
            <div className="font-semibold text-[#0a1e3f]">Edificios</div>
            <div className="text-sm text-[#6b7a99]">{data.edificios.used} / {fmtMax(data.edificios.max)}</div>
          </div>
      ...
      {data.porEdificio.map((e) => (
        <Card key={e.id}>
          <div className="p-5 space-y-3">
            <div className="font-semibold text-[#0a1e3f] flex items-center gap-2">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-[#6b7a99]">{e.unidades.used} / {fmtMax(e.unidades.max)}</span>
              </div>
              <Progress value={pct(e.unidades.used, e.unidades.max)} />
              <div className="text-xs text-[#6b7a99]">Puedes crear <b>{remaining(e.unidades.used, e.unidades.max)}</b> unidades más</div>
            </div>

            <div className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                <span className="text-[#5a4030]">Administradores</span>
                <span className="text-[#6b7a99]">{e.admins.used} / {fmtMax(e.admins.max)}</span>
              </div>
              <Progress value={pct(e.admins.used, e.admins.max)} />
              <div className="text-xs text-[#6b7a99]">Puedes invitar a <b>{remaining(e.admins.used, e.admins.max)}</b> admins más</div>
            </div>
          </div>
        </Card>
      ))}
      ...
      {data.porEdificio.length === 0 && (
        <Card><div className="p-4 text-sm text-[#6b7a99]">Crea tu primer edificio para ver los límites por unidad/admin.</div></Card>
      )}
    </div>
  );
}
