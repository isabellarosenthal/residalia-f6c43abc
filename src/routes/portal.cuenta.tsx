  ...
  return (
    <div className="space-y-5">
      <div className={`rounded-2xl p-5 ${totalPend > 0 ? "bg-[#fde8e2] border border-[#f5b8a8]" : "bg-[#e8f5e9] border border-[#a5d6a7]"}`}>
        <div className="flex items-center gap-2 text-xs text-[#6b7a99]"><Wallet className="w-4 h-4" />Saldo pendiente</div>
        <div className="font-display font-extrabold text-3xl text-[#0a1e3f] mt-1">{fmt(totalPend, moneda)}</div>
      ...
      <div>
        <h2 className="font-display font-extrabold text-lg text-[#0a1e3f] mb-2">Pendientes</h2>
        {pendientes.length === 0 ? (
          <div className="text-sm text-[#6b7a99] bg-white border border-[#e8ecf3] rounded-2xl p-5">No tienes cobros pendientes.</div>
        ) : (
          <div className="space-y-2">
            {pendientes.map((c) => (
              <div key={c.id} className="bg-white border border-[#e8ecf3] rounded-xl p-4 flex items-center justify-between">
                <div>
                  <div className="font-semibold text-[#0a1e3f]">{c.concepto}</div>
                  <div className="text-xs text-[#6b7a99]">Vence {fmtD(c.fecha_vencimiento)}</div>
                </div>
                <div className="text-right">
                  <div className="font-bold text-[#0a1e3f]">{fmt(Number(c.monto), moneda)}</div>
      ...
      {pagados.length > 0 && (
        <div>
          <h2 className="font-display font-extrabold text-lg text-[#0a1e3f] mb-2">Historial</h2>
          <div className="space-y-2">
            {pagados.slice(0, 12).map((c) => (
              <div key={c.id} className="bg-white border border-[#e8ecf3] rounded-xl p-3 flex items-center justify-between text-sm">
                <div>
                  <div className="text-[#0a1e3f]">{c.concepto}</div>
                  <div className="text-xs text-[#6b7a99]">Pagado {c.fecha_pago ? fmtD(c.fecha_pago) : "—"}</div>
                </div>
      ...
      <p className="text-xs text-[#6b7a99] text-center pt-2">
