...
function GuardiaLayout() {
  ...
  if (loading || !user) {
    return <div className="min-h-screen flex items-center justify-center bg-[#1a0f08]"><div className="text-[#E8E8E8] text-sm">Cargando…</div></div>;
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#ffffff] px-4">
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 max-w-md text-center">
          <ShieldCheck className="w-10 h-10 mx-auto text-[#4A154B] mb-2" />
          <div className="font-semibold text-[#4A154B] mb-1">Acceso restringido</div>
          <p className="text-sm text-[#64748B]">Esta vista es solo para personal de seguridad.</p>
          <Link to="/" className="text-sm text-[#4A154B] hover:underline mt-3 inline-block">Ir al inicio</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#1a0f08] text-white">
      <header className="bg-[#4A154B] border-b border-[#350d36] sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          <Link to="/guardia" className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-lg bg-[#4A154B] flex items-center justify-center"><ShieldCheck className="w-5 h-5 text-white" /></div>
            <div className="font-display font-extrabold text-lg">Vigilancia</div>
          </Link>
          <div className="flex items-center gap-3">
            <span className="text-sm text-[#E8E8E8] hidden sm:inline">{profile?.full_name ?? user.email}</span>
