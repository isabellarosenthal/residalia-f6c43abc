    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "linear-gradient(135deg,#ffffff 0%,#F8FAFC 100%)" }}>
      <div className="w-full max-w-md bg-white shadow-lg rounded-3xl p-10">
        <div className="flex items-center gap-3 justify-center mb-2">
          <div className="w-11 h-11 rounded-xl bg-[#2D3748] flex items-center justify-center">
            <Building className="w-6 h-6 text-white" />
          </div>
          <div className="font-display font-extrabold text-2xl text-[#0a1e3e]">
            <span className="text-[#2D3748]">Altura</span><span className="text-[#2D3748]">Cloud</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 p-1 bg-[#F8F7F5] rounded-full mb-6">
          <button
            type="button"
            onClick={() => { navigate({ to: "/login", search: {} }); }}
            className={`text-sm font-semibold py-2 rounded-full transition ${!isResidenteFlow ? "bg-[#2D3748] text-white shadow" : "text-[#2D3748] hover:bg-white/60"}`}
          >
            Administrador
          </button>
          <button
        ...
        <form onSubmit={submit} className="space-y-4">
          {mode === "signup" && (
            <>
              {!isResidenteFlow && (
                <div>
                  <label className="block text-sm font-medium text-[#2D3748] mb-1.5">Soy</label>
                  <div className="grid grid-cols-3 gap-2">
                    {([["admin_condominio", "Admin"], ["residente", "Residente"], ["guardia", "Guardia"]] as const).map(([v, l]) => (
                      <button key={v} type="button" onClick={() => setSignupRole(v)}
                        className={`text-sm py-2 rounded-lg border ${signupRole === v ? "bg-[#2D3748] text-white border-[#D97757]" : "border-[#EBC988] text-[#2D3748] hover:border-[#D97757]"}`}>
                        {l}
                      </button>
                    ))}
                  </div>
              ...
              {signupRole === "residente" && (
                <div>
                  <label className="block text-sm font-medium text-[#2D3748] mb-1.5">Código de invitación</label>
                  <input value={invitationCode} onChange={(e) => setInvitationCode(e.target.value.toUpperCase())} required
                    placeholder="ABC123" maxLength={6}
                    className="w-full border border-[#EBC988] rounded-xl px-4 py-2.5 text-[#2D3748] font-mono tracking-widest uppercase outline-none focus:border-[#D97757] focus:ring-2 focus:ring-[#D97757]/20" />
                  <p className="text-xs text-[#64748B] mt-1.5">Tu administrador te lo envió por WhatsApp o correo.</p>
                </div>
              )}
              <div>
                <label className="block text-sm font-medium text-[#2D3748] mb-1.5">Nombre completo</label>
                <input value={name} onChange={(e) => setName(e.target.value)} required
                  className="w-full border border-[#EBC988] rounded-xl px-4 py-2.5 text-[#2D3748] outline-none focus:border-[#D97757] focus:ring-2 focus:ring-[#D97757]/20" />
              </div>
            </>
          )}
          <div>
            <label className="block text-sm font-medium text-[#2D3748] mb-1.5">Correo electrónico</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="tu@email.com"
              className="w-full border border-[#EBC988] rounded-xl px-4 py-2.5 text-[#2D3748] placeholder:text-[#64748B] outline-none focus:border-[#D97757] focus:ring-2 focus:ring-[#D97757]/20" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[#2D3748] mb-1.5">Contraseña</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6}
              className="w-full border border-[#EBC988] rounded-xl px-4 py-2.5 text-[#2D3748] outline-none focus:border-[#D97757] focus:ring-2 focus:ring-[#D97757]/20" />
          </div>
          <button type="submit" disabled={busy}
            className="w-full bg-[#2D3748] hover:bg-[#1F2937] text-white font-semibold py-3 rounded-full transition-colors disabled:opacity-60">
            {busy ? "Procesando…" : mode === "login" ? "Iniciar sesión" : "Crear cuenta"}
          </button>
        </form>

        <button onClick={() => setMode(mode === "login" ? "signup" : "login")}
          className="w-full mt-4 text-sm text-[#64748B] hover:text-[#2D3748] transition-colors">
          {mode === "login" ? "¿No tienes cuenta? Crear una" : "¿Ya tienes cuenta? Iniciar sesión"}
        </button>

        <div className="mt-6 text-center text-xs text-[#64748B]">
          <Link to="/" className="hover:text-[#2D3748]">Volver al inicio</Link>
        </div>
      </div>
    </div>
  );
