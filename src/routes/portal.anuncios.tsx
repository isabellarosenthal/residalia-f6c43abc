...
function Anuncios() {
  ...
  return (
    <div className="space-y-3">
      <h1 className="font-display font-extrabold text-xl text-[#374151]">Anuncios del edificio</h1>
      {items.length === 0 ? (
        ...
        <article key={a.id} className="bg-white border border-[#E2E8F0] rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-1">
            {a.tipo && <span className="text-[10px] uppercase tracking-wide bg-[#fde8e2] text-[#374151] px-2 py-0.5 rounded-full">{a.tipo}</span>}
            <span className="text-xs text-[#64748B]">{fmtDT(a.created_at)}</span>
          </div>
          <h2 className="font-display font-bold text-lg text-[#374151]">{a.titulo}</h2>
          {a.cuerpo && <p className="text-sm text-[#374151] mt-1 whitespace-pre-wrap">{a.cuerpo}</p>}
        </article>
      ))}
    </div>
  );
