    ...
    <AppShell>
      <div className="space-y-5 max-w-[1500px] mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="font-display font-extrabold text-2xl text-[#173B7A]">Propiedades en Venta / Renta</h1>
          ...
          <div className="inline-flex rounded-lg border border-[#E2E8F0] bg-white p-0.5">
            <Button size="sm" variant="ghost" onClick={() => setView("table")}
              className={`h-8 px-3 ${view === "table" ? "bg-[#F8FAFC] text-[#173B7A]" : "text-[#64748B]"}`}>
              <TableIcon className="w-4 h-4 mr-1" /> Tabla
            </Button>
            <Button size="sm" variant="ghost" onClick={() => setView("cards")}
              className={`h-8 px-3 ${view === "cards" ? "bg-[#F8FAFC] text-[#173B7A]" : "text-[#64748B]"}`}>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 bg-white border border-dashed border-[#E2E8F0] rounded-2xl">
            ...
            <p className="font-display font-semibold text-[#173B7A]">No hay propiedades publicadas</p>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-[#F8FAFC] hover:bg-[#F8FAFC]">
                      ...
                      <TableHead className="text-[#173B7A] font-semibold text-right">Venta</TableHead>
                      <TableHead className="text-[#173B7A] font-semibold text-right">Renta/mes</TableHead>
                    ...
                    {filtered.map((u) => {
                      ...
                      return (
                        <TableRow key={u.id}>
                          ...
                          <TableCell className="text-sm text-[#173B7A]">
                            {ed?.nombre ?? "—"}
                          ...
                          <TableCell className="text-right text-sm font-semibold text-[#173B7A]">
                            {u.precio_venta ? fmtL(u.precio_venta) : <span className="text-[#64748B] font-normal">—</span>}
                          </TableCell>
                          <TableCell className="text-right text-sm text-[#173B7A]">
