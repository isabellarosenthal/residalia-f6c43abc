    ...
    <AppShell>
      <div className="space-y-5 max-w-[1500px] mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="font-display font-extrabold text-2xl text-[#1e1b4b]">Propiedades en Venta / Renta</h1>
            <p className="text-sm text-[#8b8bb5]">Catálogo comercial de unidades publicadas</p>
          </div>
          <div className="inline-flex rounded-lg border border-[#e0e7ff] bg-white p-0.5">
            <Button size="sm" variant="ghost" onClick={() => setView("table")}
              className={`h-8 px-3 ${view === "table" ? "bg-[#eef2ff] text-[#1e1b4b]" : "text-[#8b8bb5]"}`}>
              <TableIcon className="w-4 h-4 mr-1" /> Tabla
            </Button>
            <Button size="sm" variant="ghost" onClick={() => setView("cards")}
              className={`h-8 px-3 ${view === "cards" ? "bg-[#eef2ff] text-[#1e1b4b]" : "text-[#8b8bb5]"}`}>
        ...
        <div className="flex flex-wrap gap-2 bg-white border border-[#e0e7ff] rounded-xl p-3">
        ...
        {isLoading ? (
          <div className="text-center text-[#8b8bb5] py-12">Cargando propiedades…</div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-16 bg-white border border-dashed border-[#e0e7ff] rounded-2xl">
            <Tag className="w-10 h-10 mx-auto mb-3 text-[#c4a896]" />
            <p className="font-display font-semibold text-[#1e1b4b]">No hay propiedades publicadas</p>
            <p className="text-sm text-[#8b8bb5] mt-1">
              {unidades.length === 0
          ...
          <>
            <p className="text-xs text-[#8b8bb5]">{filtered.length} {filtered.length === 1 ? "propiedad" : "propiedades"}</p>
            {view === "cards" ? (
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-[#eef2ff] hover:bg-[#eef2ff]">
                      ...
                      <TableHead className="text-[#1e1b4b] font-semibold">Edificio</TableHead>
                      <TableHead className="text-[#1e1b4b] font-semibold">Tipo</TableHead>
                      <TableHead className="text-[#1e1b4b] font-semibold">Características</TableHead>
                      <TableHead className="text-[#1e1b4b] font-semibold">Estado</TableHead>
                      <TableHead className="text-[#1e1b4b] font-semibold text-right">Venta</TableHead>
                      <TableHead className="text-[#1e1b4b] font-semibold text-right">Renta/mes</TableHead>
                  ...
                  <TableBody>
                    {filtered.map((u) => {
                      ...
                      return (
                        <TableRow key={u.id}>
                          <TableCell>
                            <div className="font-semibold text-[#1e1b4b]">#{u.numero}</div>
                            <div className="text-xs text-[#8b8bb5]">{u.piso != null ? `Piso ${u.piso}` : "—"}</div>
                          </TableCell>
                          <TableCell className="text-sm text-[#1e1b4b]">
                            {ed?.nombre ?? "—"}
                            <div className="text-xs text-[#8b8bb5]">{ed?.ciudad ?? ""}</div>
                          </TableCell>
                          <TableCell className="capitalize text-sm">{u.tipo ?? "—"}</TableCell>
                          <TableCell>
                            <div className="flex items-center gap-3 text-xs text-[#312e81]">
                              <span className="flex items-center gap-1"><BedDouble className="w-3.5 h-3.5 text-[#8b8bb5]" />{u.habitaciones ?? 0}</span>
                              <span className="flex items-center gap-1"><Bath className="w-3.5 h-3.5 text-[#8b8bb5]" />{u.banos ?? 0}</span>
                              <span className="flex items-center gap-1"><Car className="w-3.5 h-3.5 text-[#8b8bb5]" />{u.parqueos ?? 0}</span>
                            </div>
                            {u.area_m2_construccion && <div className="text-xs text-[#8b8bb5] mt-0.5">{u.area_m2_construccion} m²</div>}
                          </TableCell>
                          <TableCell><EstadoComercialBadge value={u.estado_comercial} /></TableCell>
                          <TableCell className="text-right text-sm font-semibold text-[#818cf8]">
                            {u.precio_venta ? fmtL(u.precio_venta) : <span className="text-[#8b8bb5] font-normal">—</span>}
                          </TableCell>
                          <TableCell className="text-right text-sm text-[#1e1b4b]">
                            {u.precio_renta ? fmtL(u.precio_renta) : <span className="text-[#8b8bb5]">—</span>}
                          </TableCell>
