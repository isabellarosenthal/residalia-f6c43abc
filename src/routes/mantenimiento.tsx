    ...
    <AppShell>
      <div className="space-y-5 max-w-[1400px] mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="font-display font-extrabold text-2xl text-[#1e1b4b]">Mantenimiento</h1>
            <p className="text-sm text-[#8b8bb5]">Incidencias, órdenes de trabajo y proveedores</p>
          </div>
        ...
        <Tabs defaultValue="incidencias">
          ...
          <TabsContent value="incidencias" className="space-y-3 pt-4">
            <Card className="p-3 flex flex-wrap gap-2 items-center">
              ...
              <Button onClick={() => { setIncEdit(null); setIncOpen(true); }} disabled={edificioId === "all"} className="bg-[#818cf8] hover:bg-[#6366f1]">
            ...
            <Card className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {incFiltered.length === 0 && <tr><td colSpan={6} className="text-center text-[#8b8bb5] py-8">Sin incidencias</td></tr>}
                  {incFiltered.map((i) => (
                    <tr key={i.id} className="border-t border-[#f0e6e0]">
                      ...
                      <td className="p-3 text-right whitespace-nowrap">
                        ...
                        <Button size="icon" variant="ghost" onClick={() => confirm("¿Eliminar?") && delInc.mutate(i.id)}><Trash2 className="w-4 h-4 text-[#be185d]" /></Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <TabsContent value="ordenes" className="space-y-3 pt-4">
            <Card className="p-3 flex flex-wrap gap-2 items-center">
              ...
              <Button onClick={() => { setOrdEdit(null); setOrdOpen(true); }} disabled={edificioId === "all"} className="bg-[#818cf8] hover:bg-[#6366f1]">
            ...
            <Card className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {ordFiltered.length === 0 && <tr><td colSpan={7} className="text-center text-[#8b8bb5] py-8">Sin órdenes</td></tr>}
                  {ordFiltered.map((o) => (
                    <tr key={o.id} className="border-t border-[#f0e6e0]">
                      ...
                      <td className="p-3 text-right whitespace-nowrap">
                        ...
                        <Button size="icon" variant="ghost" onClick={() => confirm("¿Eliminar?") && delOrd.mutate(o.id)}><Trash2 className="w-4 h-4 text-[#be185d]" /></Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <TabsContent value="proveedores" className="space-y-3 pt-4">
            <Card className="p-3 flex flex-wrap gap-2 items-center">
              ...
              <Button onClick={() => { setProvEdit(null); setProvOpen(true); }} className="bg-[#818cf8] hover:bg-[#6366f1]">
                <Plus className="w-4 h-4 mr-1" />Nuevo proveedor
              </Button>
            </Card>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {provFiltered.length === 0 && <div className="col-span-full text-center text-[#8b8bb5] py-8"><Users className="w-8 h-8 mx-auto mb-2" />Sin proveedores</div>}
              {provFiltered.map((p) => (
                <Card key={p.id} className="p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="font-semibold text-[#1e1b4b]">{p.nombre}</div>
                      <div className="text-xs text-[#8b8bb5]">{p.servicio ?? "—"}</div>
                    </div>
                    {p.calificacion && <div className="flex items-center gap-1 text-sm text-[#818cf8]"><Star className="w-4 h-4 fill-current" />{Number(p.calificacion).toFixed(1)}</div>}
                  </div>
                  ...
                  <div className="mt-3 flex gap-1 justify-end">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => confirm("¿Eliminar?") && delProv.mutate(p.id)}><Trash2 className="w-4 h-4 text-[#be185d]" /></Button>
                  </div>
                </Card>
              ))}
            </div>
