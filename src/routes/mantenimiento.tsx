    ...
    <AppShell>
      <div className="space-y-5 max-w-[1400px] mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="font-display font-extrabold text-2xl text-[#2D3748]">Mantenimiento</h1>
        ...
        <Tabs defaultValue="incidencias">
          ...
          <TabsContent value="incidencias" className="space-y-3 pt-4">
            <Card className="p-3 flex flex-wrap gap-2 items-center">
              ...
              <Button onClick={() => { setIncEdit(null); setIncOpen(true); }} disabled={edificioId === "all"} className="bg-[#2D3748] hover:bg-[#1F2937]">
          ...
          <TabsContent value="ordenes" className="space-y-3 pt-4">
            <Card className="p-3 flex flex-wrap gap-2 items-center">
              ...
              <Button onClick={() => { setOrdEdit(null); setOrdOpen(true); }} disabled={edificioId === "all"} className="bg-[#2D3748] hover:bg-[#1F2937]">
          ...
          <TabsContent value="proveedores" className="space-y-3 pt-4">
            <Card className="p-3 flex flex-wrap gap-2 items-center">
              ...
              <Button onClick={() => { setProvEdit(null); setProvOpen(true); }} className="bg-[#2D3748] hover:bg-[#1F2937]">
              ...
              {provFiltered.map((p) => (
                <Card key={p.id} className="p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="font-semibold text-[#2D3748]">{p.nombre}</div>
                      <div className="text-xs text-[#64748B]">{p.servicio ?? "—"}</div>
                    </div>
                    {p.calificacion && <div className="flex items-center gap-1 text-sm text-[#2D3748]"><Star className="w-4 h-4 fill-current" />{Number(p.calificacion).toFixed(1)}</div>}
                  </div>
