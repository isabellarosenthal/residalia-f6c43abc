...
function Reservar() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="font-display font-extrabold text-xl text-[#2D3748]">Reservar área común</h1>
      ...
      <form onSubmit={submit} className="space-y-4 bg-white border border-[#E2E8F0] rounded-2xl p-5">
        ...
        <Button type="submit" disabled={save.isPending || !!conflicto || !areaId} className="w-full bg-[#2D3748] hover:bg-[#1F2937]">
      ...
      {proximas.length > 0 && (
        <div>
          <h2 className="font-display font-extrabold text-lg text-[#2D3748] mb-2">Mis próximas reservas</h2>
            ...
            {proximas.map((r) => {
              ...
              return (
                <div key={r.id} className="bg-white border border-[#E2E8F0] rounded-xl p-3 flex items-center gap-3">
                  <CalendarCheck className="w-5 h-5 text-[#2D3748]" />
                  <div className="flex-1">
                    <div className="font-semibold text-[#2D3748]">{a?.nombre ?? "Área"}</div>
                    <div className="text-xs text-[#64748B]">{fmtDT(r.fecha_inicio)} – {new Date(r.fecha_fin).toLocaleTimeString("es-HN", { timeStyle: "short" })}</div>
                  </div>
                  <span className="text-[10px] uppercase tracking-wide bg-[#fde8e2] text-[#2D3748] px-2 py-0.5 rounded-full">{r.estado}</span>
                </div>
              );
            })}
          </div>
