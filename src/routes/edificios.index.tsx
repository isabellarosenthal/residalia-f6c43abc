    ...
    <AppShell>
      <div className="space-y-6 max-w-[1400px] mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="font-display font-extrabold text-2xl text-[#173B7A]">Edificios</h1>
            <p className="text-sm text-[#64748B]">Administra todos tus condominios y residenciales desde un solo lugar</p>
          </div>
          <Button onClick={() => setOpen(true)} className="bg-[#173B7A] hover:bg-[#0f2659] text-white">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="inline-flex rounded-lg border border-[#E2E8F0] bg-white overflow-hidden">
            <button type="button" onClick={() => setView("grid")} className={`px-3 py-2 text-sm flex items-center gap-1 ${view === "grid" ? "bg-[#173B7A] text-white" : "text-[#1E293B] hover:bg-[#F1F5F9]"}`}><LayoutGrid className="w-4 h-4" />Tarjetas</button>
            <button type="button" onClick={() => setView("table")} className={`px-3 py-2 text-sm flex items-center gap-1 ${view === "table" ? "bg-[#173B7A] text-white" : "text-[#1E293B] hover:bg-[#F1F5F9]"}`}><List className="w-4 h-4" />Tabla</button>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={edificios.length === 0 ? <Button onClick={() => setOpen(true)} className="bg-[#173B7A] hover:bg-[#0f2659]"><Plus className="w-4 h-4 mr-1" />Crear edificio</Button> : null}
          />
        ) : view === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
