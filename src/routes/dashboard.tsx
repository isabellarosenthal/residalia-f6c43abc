...
function DashboardPage() {
  ...
  const estadoPagos = [
    ...
    { name: "Pendientes", value: cobros.filter((c) => c.estado === "pendiente").length, color: "#D9A441" },
    { name: "Morosos", value: cobros.filter((c) => c.estado === "vencido").length, color: "#be185d" },
    { name: "Parcial", value: cobros.filter((c) => c.estado === "parcial").length, color: "#173B7A" },
  ].filter((e) => e.value > 0);
      ...
      <div className="space-y-6 max-w-[1400px] mx-auto">
          ...
          {edificios.length > 1 && (
            <div className="flex items-center gap-2 bg-white border border-[#E2E8F0] rounded-full p-1">
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${edificioId === "all" ? "bg-[#173B7A] text-white" : "text-[#1E293B] hover:bg-[#F8FAFC]"}`}
              >
                Todos
              </button>
              {edificios.map((e) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${edificioId === e.id ? "bg-[#173B7A] text-white" : "text-[#1E293B] hover:bg-[#F8FAFC]"}`}
                >
                  {e.nombre}
                </button>
              ))}
        ...
        {edificios.length === 0 ? (
          <Card className="p-10 text-center">
            <Building2 className="w-12 h-12 text-[#D9A441] mx-auto mb-3" />
            <h3 className="font-display font-bold text-[#173B7A] text-lg">Aún no tenés edificios</h3>
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <Card className="p-5">
                <h3 className="font-display font-bold text-[#173B7A] mb-1">Estado de cobros</h3>
