...
function PortalLayout() {
  ...
  const active = { className: linkBase + " text-[#173B7A] bg-[#fde8e2]" };
        ...
        <header className="bg-white border-b border-[#E2E8F0] sticky top-0 z-10">
          <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between gap-2">
            <Link to="/portal" className="flex items-center gap-2 min-w-0">
              <div className="w-9 h-9 rounded-lg bg-[#173B7A] flex items-center justify-center shrink-0"><Building className="w-5 h-5 text-white" /></div>
              <div className="font-display font-extrabold text-lg truncate"><span className="text-[#173B7A]">Portal</span><span className="text-[#173B7A]"> residente</span></div>
            </Link>
            <div className="flex items-center gap-2 shrink-0">
              <InstallAppButton />
              <button onClick={() => signOut()} className="text-[#64748B] hover:text-[#173B7A] p-1.5" aria-label="Salir"><LogOut className="w-5 h-5" /></button>
            </div>
          </div>
