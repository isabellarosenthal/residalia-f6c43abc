...
function PasesHoy() {
  ...
  if (isLoading) return <div className="text-sm text-[#c9b8b0]">Cargando…</div>;

  return (
    <div className="space-y-2">
      <h2 className="font-display font-extrabold text-lg mb-2">Pases del día ({delDia.length})</h2>
      {delDia.length === 0 ? (
        <div className="bg-[#2d1200] border border-[#3d2410] rounded-2xl p-6 text-center text-[#c9b8b0]">Sin pases hoy.</div>
      ) : delDia.map((p) => {
        const usados = p.usos_actuales ?? 0; const max = p.usos_maximos ?? 1;
        const tone = p.fecha_salida ? "neutral" : usados >= max ? "danger" : "success";
        return (
          <div key={p.id} className="bg-[#2d1200] border border-[#3d2410] rounded-2xl p-4 flex items-center justify-between gap-3">
            <div>
              <div className="font-semibold text-white">{p.visitante_nombre}</div>
              <div className="text-xs text-[#c9b8b0] font-mono">{p.qr_code}</div>
              <div className="text-xs text-[#c9b8b0] mt-1">{fmtDT(p.fecha_entrada)}</div>
            </div>
