...
function PasesHoy() {
  ...
  return (
    <div className="space-y-2">
      ...
      {delDia.length === 0 ? (
        <div className="bg-[#2D3748] border border-[#1F2937] rounded-2xl p-6 text-center text-[#EBC988]">Sin pases hoy.</div>
      ) : delDia.map((p) => {
        const usados = p.usos_actuales ?? 0; const max = p.usos_maximos ?? 1;
        const tone = p.fecha_salida ? "neutral" : usados >= max ? "danger" : "success";
        return (
          <div key={p.id} className="bg-[#2D3748] border border-[#1F2937] rounded-2xl p-4 flex items-center justify-between gap-3">
            <div>
