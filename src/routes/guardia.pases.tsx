...
function PasesHoy() {
  ...
  return (
    <div className="space-y-2">
      ...
      {delDia.length === 0 ? (
        <div className="bg-[#4A154B] border border-[#350d36] rounded-2xl p-6 text-center text-[#E8E8E8]">Sin pases hoy.</div>
      ) : delDia.map((p) => {
        const usados = p.usos_actuales ?? 0; const max = p.usos_maximos ?? 1;
        const tone = p.fecha_salida ? "neutral" : usados >= max ? "danger" : "success";
        return (
          <div key={p.id} className="bg-[#4A154B] border border-[#350d36] rounded-2xl p-4 flex items-center justify-between gap-3">
            <div>
              <div className="font-semibold text-white">{p.visitante_nombre}</div>
              <div className="text-xs text-[#E8E8E8] font-mono">{p.qr_code}</div>
              <div className="text-xs text-[#E8E8E8] mt-1">{fmtDT(p.fecha_entrada)}</div>
            </div>
